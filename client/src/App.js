...
class App extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      this.tokenKycContractInstance = new this.web3.eth.Contract(
        KycContract.abi,
  ...
  handleKycWhitelist=async() => {
    ...
    await this.tokenKycContractInstance.methods.setKycCompleted(this.state.kycAddress).send({
  ...
  render() {
    ...
    return (
      <div className="App">
        <h1>Starducks cappachino token sale!</h1>
        <p>Get your token today.</p>
        <h2>Kyc Whitelisting</h2>
          Address to Allow: <input type="text" name="kycAddress" value={this.state.kycAddress} onChange={this.handleInputChange} />  
          <button type="button" onClick={this.handleKycWhitelist}>Add to Whitelisting</button>   
          <h2>Buy Tokens</h2>
